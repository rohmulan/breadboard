...
import { BreadboardManifestBuilder, } from "@breadboard-ai/manifest";
...
const MANIFEST_PATH = path.join(MODULE_DIR, "..");
const GRAPH_PATH = path.join(MODULE_DIR, "..", "example-boards");
...
async function saveTypeScriptBoard(filePath) {
    try {
        ...
        if ("inputs" in module.default && "outputs" in module.default) {
            ...
            await writeFile(path.join(graphDir, jsonFile), JSON.stringify(serialize(board), null, 2));
            return manifest;
        }
        else {
            const board = module.default;
            ...
            await writeFile(path.join(graphDir, jsonFile), JSON.stringify(formatGraphDescriptor(JSON.parse(JSON.stringify(board))), null, 2));
            return manifest;
        }
    }
    catch (e) {
...
async function saveJsonBoard(filePath) {
    const relativePath = path.relative(PATH, filePath);
    ...
    try {
        ...
        const file = JSON.parse(boardData);
        const manifest = {
...
async function saveAllBoards(prefix) {
    ...
    await writeFile(path.join(MANIFEST_PATH, `${prefix}-boards.json`), JSON.stringify(manifest, null, 2));
}
await Promise.all([saveAllBoards("playground"), saveAllBoards("examples")]);
//# sourceMappingURL=make-graphs.js.map