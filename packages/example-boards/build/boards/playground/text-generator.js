 ...
 */
import { anyOf, board, enumeration, input, object, output, serialize, } from "@breadboard-ai/build";
...
const switchModel = code({ model, serialized: JSON.stringify(geminiGenerator) }, { board: anyOf(object({ kind: "string", board: "string" }), object({ kind: "string", path: "string" })), model: "string" }, ({ model, serialized }) => {
    if (model == "Gemini Pro") {
